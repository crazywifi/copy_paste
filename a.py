...
def Search_Resume():
    ...
    # Search Entry Field
    tk.Label(search_window, text="Enter search keywords:").pack(pady=5)
    ...
    # Folder Display
    global folder_label  
    folder_label = tk.Label(search_window, text=f"Folder: {RESUME_FOLDER}")
...
label = tk.Label(root, text="Select Email txt File:")
label.pack(pady=10)

email_id_entry = tk.Entry(root, width=40)
email_id_entry.pack(pady=5)

label = tk.Label(root, text="Follow steps one by one")
label.pack(pady=5)
...
label = tk.Label(root, text="For any feedback: rishabhsharma96@deloitte.com",fg="red", cursor="hand2",font=("Arial",10,"underline"))
label.pack(pady=2)
